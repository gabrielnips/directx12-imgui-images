...
void ImGuiManager::NewFrame()
{
    ...
    ImGui::Begin("##fps", NULL, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
    ...
    if (ImGui::Button("Load Image", ImVec2(-1, 0)))
    {
        ...
        if (!path_str.empty())
        {
            if (s_loadedTextures.count(path_str) == 0)
            {
    ...
    for (const auto& pair : s_loadedTextures)
    {
        ...
        if (ImGui::Begin(imagePath.c_str()))
        {
            ...
            {
                ImVec2 displaySize = ImVec2((float)texture.Width, (float)texture.Height);
                ...
                ImGui::Image((ImTextureID)texture.SrvGpuDescriptorHandle.ptr, displaySize);
            }
            else
            {
